...
#define NRF_CE_LOW()        NRF_CE=0           //CE置低
#define NRF_Read_IRQ()      NRF_IRQ=1


// 用户配置 发送和 接收地址，频道

uint8 TX_ADDRESS[5] = NRF_LINKADDRESS;//{0x34, 0x43, 0x10, 0x10, 0x01};   // 定义一个静态发送地址
uint8 RX_ADDRESS[5] = NRF_LINKADDRESS;//{0x34, 0x43, 0x10, 0x10, 0x01};
...
{
    //配置NRF管脚复用                  
    spi_init(NRF_SPIn,NRF_PCSn,MASTER,12500*1000);             //初始化SPI,主机模式
    GPIO_Init(NRF_CE_PORT, GPO, LOW);                               //初始化CE，默认进入待机模式
    port_init(NRF_IRQ_PORT, FALLING | PULL_UP);            //初始化IRQ管脚为下降沿 触发中断
...
uint8 nrf_writereg(uint8 reg, uint8 dat)
{
    ...
    spi_mosi(NRF_SPIn, NRF_PCSn, buff, buff, 2); //发送buff里数据，并采集到 buff里
...
uint8 nrf_readreg(uint8 reg, uint8 *dat)
{
    ...
    spi_mosi(NRF_SPIn, NRF_PCSn, buff, buff, 2); //发送buff数据，并保存到buff里
...
{
    spi_mosi_cmd(NRF_SPIn, NRF_PCSn, &reg , NULL, pBuf, NULL, 1 , len); //发送 reg ，pBuf 内容，不接收
...
{
    spi_mosi_cmd(NRF_SPIn, NRF_PCSn, &reg , NULL, NULL, pBuf, 1 , len); //发送reg，接收到buff
...
#define NRF_CHECH_DATA  0xC2        //此值为校验数据时使用，可修改为其他值
    ...
    spi_mosi_cmd(NRF_SPIn, NRF_PCSn, &reg, NULL , buff, NULL, 1 , 5); //写入校验数据

    reg = TX_ADDR;
    spi_mosi_cmd(NRF_SPIn, NRF_PCSn, &reg, NULL , NULL, buff, 1 , 5); //读取校验数据
...
void nrf_rx_mode(void)
{
    ...
    nrf_writereg(NRF_WRITE_REG + NRF_CONFIG, 0x0B | (IS_CRC16 << 2));       //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式
...
void nrf_tx_mode(void)
{
    ...
    nrf_writereg(NRF_WRITE_REG + NRF_CONFIG, 0x0A | (IS_CRC16 << 2)); //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,发射模式,开启所有中断
...
void fire_cpy( uint8 *dst, uint8 *src, uint32 count)    //nRF_rx中用到
{
  uint32 n;
  if(count != 0)
  {
